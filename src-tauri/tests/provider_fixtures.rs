...
fn load_fixture(name: &str) -> String {
    ...
    std::fs::read_to_string(&path).unwrap_or_else(|_| {
        panic!("Fixture file not found: {}", path)
    })
}
...
fn parse_full_5h_7d_response() {
    ...
    // Verify 5h window
    let window_5h = snapshot.windows.iter().find(|w| w.window == "5h").expect("5h window missing");
    assert!((window_5h.remaining_percent - 72.5).abs() < f32::EPSILON);
    assert_eq!(window_5h.reset_at, Some("2026-07-16T08:00:00Z".to_string()));

    // Verify 7d window
    let window_7d = snapshot.windows.iter().find(|w| w.window == "7d").expect("7d window missing");
...
fn clamp_out_of_range_percentages() {
    ...
    let result = parse_provider_response(&json);
    assert!(result.is_ok(), "Expected Ok with clamping, got Err: {:?}", result);
    let snapshot = result.unwrap();
    assert_eq!(snapshot.windows.len(), 2);
...
fn reject_duplicate_windows() {
    ...
    let result = parse_provider_response(&json);
    assert!(result.is_err(), "Expected Err for duplicate windows, got Ok: {:?}", result);
    match result {
...
fn filter_unsupported_windows() {
    ...
    let result = parse_provider_response(&json);
    assert!(result.is_ok(), "Expected Ok (unsupported filtered), got Err: {:?}", result);
    let snapshot = result.unwrap();
    assert_eq!(snapshot.windows.len(), 1);
...
fn handle_failed_response() {
    ...
    let result = parse_provider_response(&json);
    assert!(result.is_err(), "Expected Err for failed response, got Ok: {:?}", result);
    match result {
...
fn handle_empty_windows_array() {
    ...
    let result = parse_provider_response(&json);
    assert!(result.is_ok(), "Expected Ok for empty windows, got Err: {:?}", result);
    let snapshot = result.unwrap();
...
fn clamp_boundary_values() {
    ...
    assert!(result.is_err(), "Expected duplicate error for boundary test");
}
